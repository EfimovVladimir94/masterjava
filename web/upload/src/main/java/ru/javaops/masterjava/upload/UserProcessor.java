...
public class UserProcessor {
    ...
    public static UserDao dao = DBIProvider.getDao(UserDao.class);
    private ExecutorService executor = Executors.newFixedThreadPool(THREAD_NUMBERS);
    ...
    public List<FailedChunk> process(final InputStream is, int chunkSize) throws XMLStreamException, JAXBException {
        ...
        return new Callable<List<FailedChunk>>() {
            ...
            public List<FailedChunk> call() throws XMLStreamException, JAXBException {
                ...
                futures.forEach(cf -> {
                    try {
                        ...
                        log.info(cf.emailRange + " successfully expected");
                    } catch (Exception e) {
            ...
            private ChunkFuture submit(List<User> chunk) {
                ChunkFuture chunkFuture = new ChunkFuture(chunk, executor.submit(() -> dao.InsertAndGetConflictEmails(chunk)));
